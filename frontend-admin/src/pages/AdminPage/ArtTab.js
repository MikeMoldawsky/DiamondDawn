...
const ART_MAPPING = {
  ...
  [STAGE.CUT]: { shapes: [SHAPE.PEAR, SHAPE.ROUND, SHAPE.OVAL, SHAPE.RADIANT], setter: 'setCutVideoUrl'},
  [STAGE.POLISH]: { shapes: [SHAPE.PEAR, SHAPE.ROUND, SHAPE.OVAL, SHAPE.RADIANT], setter: 'setPolishVideoUrl'},
...
const StageArt = ({ stage }) => {
  ...
  const fetchStageArtData = async () => {
    try {
      ...
      setStageArtData(
        _.zipObject(_.map(shapes, getShapeName), stageArt)
      )
    }
    catch (e) {
    ...
    <div className="center-aligned-row stage-art">
      <div className="inputs">
        {_.map(stageArtData, (videoUrl, shapeName) => (
          <div key={`stage-art-input-${stage}-${shapeName}`} className="input-container">
            <input type="text" placeholder={shapeName} value={videoUrl} onChange={e => setVideoUrl(shapeName, e.target.value)} />
          </div>
        ))}
      </div>
      <ActionButton actionKey={`set-stage-art-${stage}`} onClick={saveVideoUrls}>SAVE</ActionButton>
    </div>
  );
}
