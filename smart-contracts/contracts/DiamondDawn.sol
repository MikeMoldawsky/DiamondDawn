    ...
    IDiamondDawnAdmin
{
    ...
    function rebirth(uint tokenId, bytes calldata signature) external isDawnAllowed(tokenId) {
        require(_isValid(signature, tokenId), "Not allowed to rebirth");
    ...
    function _isValid(bytes calldata signature, uint tokenId) private view returns (bool) {
        return
            _signer ==
            keccak256(
                abi.encodePacked(
                    "\x19Ethereum Signed Message:\n32",
                    bytes32(bytes32(abi.encodePacked(_msgSender(), uint96(tokenId))))
                )
            ).recover(signature);
    }
