...
const DiamondPicker = () => {
  ...
  const selectedIndex = _.findIndex(actionableTokens, token => token.id === selectedTokenId)

  const onChange = (index) => dispatch(setSelectedTokenId(_.get(actionableTokens, index)?.id))

  return (
    <Carousel selectedItem={selectedIndex} onChange={onChange} showStatus={false} showThumbs={false} showIndicators={_.size(actionableTokens) > 1}>
