...
const Cut = () => {
  ...
  const renderContent = () => {
    if (showCompleteVideo)
      return (
        <VideoPlayer onEnded={() => setShowCompleteVideo(false)}>
          04 - CUTTING VIDEO
        </VideoPlayer>
      );
      ...
      return (
        <>
          ...
          <div className="leading-text">
            YOUR CUT DIAMOND NFT IS IN YOUR WALLET
          </div>
    ...
    return (
      <>
        ...
        {isStageActive && (
          <ActionButton actionKey="Cut" className="action-button" onClick={cut}>
            CUT
          </ActionButton>
        )}
        <Countdown
          date={endTime}
          text={["You have", `${isStageActive ? "to" : "until"} cut`]}
        />
      </>
    );
  };

  return <div className="action-view mine">{renderContent()}</div>;
};
